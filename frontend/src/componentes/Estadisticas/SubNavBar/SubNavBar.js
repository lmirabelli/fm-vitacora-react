...
export const SubNavBar = ({activar}) => {
    ...
    return(
        <div className="botonera-flotante bf-derecha">
            ...
            {location !== "/estadisticas" && <Link to="/estadisticas/goles/rivales" className={`btn-flotante ${activar === "rivales" ? "activo" : "desactivo"}`} style={{animationDelay: `${0.25 * 5}s`}} title="rivales">🤼​</Link>}
            {location !== "/estadisticas" && <Link to="/estadisticas/goles/minutos" className={`btn-flotante ${activar === "minutos" ? "activo" : "desactivo"}`} style={{animationDelay: `${0.25 * 6}s`}} title="minutos">⏱️​</Link>}
            {location !== "/estadisticas" && <Link to="/estadisticas/goles/block100" className={`btn-flotante ${activar === "block100" ? "activo" : "desactivo"}`} style={{animationDelay: `${0.25 * 7}s`}} title="block100">💯​</Link>}
            {location !== "/estadisticas" && <Link to="/estadisticas/goles/veteranos" className={`btn-flotante ${activar === "veteranos" ? "activo" : "desactivo"}`} style={{animationDelay: `${0.25 * 8}s`}} title="veteranos">👴🏻​</Link>}
        </div>
    )
} 