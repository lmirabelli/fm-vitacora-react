        ...
        <div className="tabla-partidos">
            {partidos.map((p,idx) => (
                <Link to={`/partidos/${p.id}`} className="partido" key={idx}>
                    ...
                    <div className="w-35">{p.rival}{p.paisRival.pais !== p.miEscudo.paisDelEquipo && <img src={p.paisRival.bandera} alt="bandera" className="bandera" />}</div>
                    <div className="w-10">{p.condicion === "local" ? "L" : p.condicion === "visitante" ? "V" : p.condicion === "neutral" ? "N" : "D"}</div>
